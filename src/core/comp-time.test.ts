...
describe("heat instants", () => {
  it("converts comp-local heat times to absolute instants in the comp timezone", () => {
    ...
    expect(start.toISOString()).toBe("2026-09-13T13:10:00.000Z");
    expect(end.toISOString()).toBe("2026-09-13T13:20:00.000Z");
  });
...
describe("comp day of an instant", () => {
  it("reports the calendar date in the comp timezone, not UTC", () => {
    const lateEvening = new Date("2026-09-14T02:00:00Z");

    expect(compDayOf(lateEvening, "America/New_York")).toBe("2026-09-13");
  });
});
