...
import { comp2026 } from "../test/comp-2026";
...
describe("the committed snapshot", () => {
  ...
  it("has a judge code for every event and lane it defines", () => {
...
describe("the 2026 comp fixture used by the behaviour tests", () => {
  const laneFor = (team: string) =>
    comp2026.events.map((event) => {
  ...
  it("is the 12 September 2026 comp: three events of five heats, eight lanes wide", () => {
    expect(comp2026.compDate).toBe("2026-09-12");
    expect(comp2026.events.map((e) => e.heats.length)).toEqual([5, 5, 5]);
    expect(comp2026.events.map((e) => e.lanes)).toEqual([8, 8, 8]);
  });

  it("has 37 distinct teams, 36 of which appear in Event 3", () => {
    const teamsIn = (eventIndex: number) => new Set(comp2026.events[eventIndex]?.heats.flatMap((h) => h.lanes.map((l) => l.team)));
    expect(teamsIn(0).size).toBe(37);
    expect(teamsIn(1).size).toBe(37);
    expect(teamsIn(2).size).toBe(36);
  });

  it("keeps the PDF corrections: Dumpys in lane 5 of Event 1 Heat 2, Fast but Questionable as transcribed", () => {
    ...
    expect(comp2026.events[2]?.heats[4]?.end).toBe("13:00");
  });
});
